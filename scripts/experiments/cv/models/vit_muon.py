...
class RMSNorm(nn.Module):
    """Simple RMSNorm implementation for small-transformer ablations."""
...
class IdentityNorm(nn.Module):
    """No-op normalization used as a small-scale control."""

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return x


def make_norm_layer(norm_mode: str, dim: int) -> nn.Module:
    """Create a normalization module for transformer ablations."""
    mode = str(norm_mode or "layernorm").strip().lower()
    if mode == "layernorm":
    ...
    if mode == "identity":
        return IdentityNorm()
    raise ValueError(f"Unsupported norm_mode={norm_mode!r}; expected layernorm, rmsnorm, or identity")
...
class MultiHeadAttentionWithQKActivation(nn.Module):
    ...
    def __init__(
        ...
        # Activation to apply after Q/K/V projections.
        # Defaults to Q+K to preserve the v1 behavior while enabling Q/K/V placement ablations for v2.
        self.qk_activation = qk_activation  # None = standard (no activation)
        valid_targets = {"q", "k", "v"}
        targets = tuple(str(t).lower() for t in qkv_activation_targets)
        if any(t not in valid_targets for t in targets):
            raise ValueError(f"Invalid qkv_activation_targets={qkv_activation_targets}; expected subset of {sorted(valid_targets)}")
        self.qkv_activation_targets = targets
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Apply activation to Q/K/V (placement ablation knob).
        if self.qk_activation is not None:
            if "q" in self.qkv_activation_targets:
                q = self.qk_activation(q)
            if "k" in self.qkv_activation_targets:
                k = self.qk_activation(k)
            if "v" in self.qkv_activation_targets:
                v = self.qk_activation(v)
...
class TransformerBlockMuon(nn.Module):
    ...
    def __init__(
        ...
        self.norm_mode = str(norm_mode or "layernorm").strip().lower()
        self.norm1 = make_norm_layer(self.norm_mode, dim)
        ...
        self.norm2 = make_norm_layer(self.norm_mode, dim)
...
class ViTMuonResearch(nn.Module):
    ...
    def __init__(
        ...
        self.qkv_activation_targets = tuple(str(t).lower() for t in qkv_activation_targets)
        self.norm_mode = str(norm_mode or "layernorm").strip().lower()
        ...
        self.blocks = nn.ModuleList([
            TransformerBlockMuon(
                ...
                qkv_activation_targets=self.qkv_activation_targets,
                dropout=dropout,
                drop_path=dpr[i],
        ...
        # Classification head
        self.norm = make_norm_layer(self.norm_mode, embed_dim)
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'qkv_activation_targets': list(self.qkv_activation_targets),
